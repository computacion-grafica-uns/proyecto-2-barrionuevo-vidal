Shader "ShaderCook_Torrance_Multitextura"
{
    Properties
    {
        ...
        // --------------------------------------------------------
        // 3) PROPIEDADES DE ILUMINACIÓN (mismos nombres que antes)
        // --------------------------------------------------------
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //--------------------------------------------------------
            // 1) SAMPLERS DE TEXTURAS Y “UNIFORMS” DEL MATERIAL
            //--------------------------------------------------------

            // Mapas
            sampler2D _BaseColorMap;
            sampler2D _MetallicMap;
            // (Si tuvieras RoughnessMap o AOMap, los añadirías aquí)

            // Parámetro Roughness (fallback)
            float  _Roughness;
            // Valor F0 si no hay MetallicMap
            float4 _F0;

            //--------------------------------------------------------
            // 2) UNIFORMES DE LUZ (con exactamente los mismos nombres)
            //--------------------------------------------------------
            ...
            //--------------------------------------------------------
            // 5) FUNCIÓN DE ATENUACIÓN (para Puntual y Spot)
            //--------------------------------------------------------
            ...
            //--------------------------------------------------------
            // 6) FRAGMENT SHADER: Cook‐Torrance MULTITEXTURA (sin normal map)
            //--------------------------------------------------------
            fixed4 frag(v2f i) : SV_Target
            {
                // 6.1) EXTRAEMOS BASE COLOR (albedo) DEL MAPA
                float3 albedo = tex2D(_BaseColorMap, i.uv).rgb;

                // 6.2) LECTOR DE VALOR METÁLICO (canal R de la textura)
                ...
                // 6.6) COOK‐TORRANCE: LUZ DIRECCIONAL
                {
                    ...
                    result += _DirLightColor.rgb 
                              * _DirLightIntensity 
                              * NdotL 
                              * (diffuseTerm + specular);
                }

                // 6.7) COOK‐TORRANCE: LUZ PUNTUAL
                {
    ...
    // En caso de que algo falle, hacemos fallback a un shader Diffuse simple:
    Fallback "Diffuse"
}
