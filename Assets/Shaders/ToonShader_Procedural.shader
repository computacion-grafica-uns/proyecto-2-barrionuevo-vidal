    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // Patrón procedural "burbuja ondulada"
                float2 posXZ = i.worldPos.xz * 5; // escala

                float2 grid = floor(posXZ);
                float2 fpos = frac(posXZ);

                float dist = distance(fpos, float2(0.5, 0.5));

                // Tiempo para animar el patrón (usar _Time.y para segundos)
                float wave = 0.2 * sin(posXZ.x * 10 + posXZ.y * 10 + _Time.y * 5);

                float radius = 0.4 + wave;

                float pattern = smoothstep(radius, radius - 0.05, dist);

                // Color base modulado por patrón (más claro dentro de "burbuja")
                float3 baseColorPattern = lerp(_Color.rgb * 0.5, _Color.rgb, pattern);

                float3 ambient = _AmbientColor.rgb * baseColorPattern;
                ...
                // ==== Luz Direccional ====
                {
                    ...
                    float3 diffD = _DirLightColor.rgb * _DirLightIntensity * diffFactor * baseColorPattern;
                ...
                // ==== Luz Puntual ====
                {
                    ...
                    float3 diffP = _PointLightColor.rgb * _PointLightIntensity * attP * diffFactor * baseColorPattern;
                ...
                // ==== Luz Spot ====
                {
                    ...
                    float3 diffS = _SpotLightColor.rgb * _SpotLightIntensity * attS * diffFactor * baseColorPattern;
